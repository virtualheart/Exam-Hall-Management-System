...
include('../connect.php');
extract($_POST);
   $sql = "INSERT INTO `allot` (`exam_id`,`room_type_id`,`class_id`,`added_date`) VALUES ('$exam_id','$room_type_id','$class_id','".date('Y-m-d')."')";
    echo $sql;
...
while ($row = mysqli_fetch_array($sr2)) {
     ...
     $s3 = "SELECT * FROM `tbl_student` WHERE classname ='".$class_id."' limit ".$start_ran.",".$end_ran+1;
     echo $s3;
 	 $sr3 = $conn->query($s3);
 	 $i=1;
